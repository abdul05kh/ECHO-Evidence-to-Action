/// Deterministic Policy Engine for ECHO
///
...
class PolicyEngine {
  static PriorityEvaluation evaluate({
    ...
    final criticalKeywords = [
      'fire', 'smoke', 'spark', 'sparking', 'electric shock', 'hazard',
      'gas leak', 'structural damage', 'emergency', 'flooding', 'exposed wire'
    ];
    ...
    final highKeywords = [
      'starts in', 'starting in', 'class in', 'exam', 'presentation',
      'lab 2', 'lab 1', 'lecture hall', '20 minutes', '15 minutes', '30 minutes',
      'not powering on', 'no power', 'urgent', 'blocked'
    ];
    for (final kw in highKeywords) {
      if (combinedText.contains(kw)) {
        return const PriorityEvaluation(
          priority: 'high',
          reason: 'Upcoming operational deadline or scheduled academic session impacted',
    ...
    final mediumKeywords = [
      'spare cable', 'alternative', 'flickering', 'low toner', 'slow',
      'chair broken', 'door handle', 'projector'
    ];
    for (final kw in mediumKeywords) {
      if (combinedText.contains(kw)) {
        return const PriorityEvaluation(
          priority: 'medium',
          reason: 'Equipment requires service before next major cycle; partial workaround may exist',
    ...
    return const PriorityEvaluation(
      priority: 'low',
      reason: 'Routine observation; no immediate deadline or safety risk detected',
